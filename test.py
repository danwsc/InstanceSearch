...
def im_detect(sess, net, layer, im, transpose_layer, boxes=None):
    ...
    if transpose_layer:
        cls_score, cls_prob, bbox_pred, rois, feat = sess.run([net.get_output('cls_score'),
                                                     net.get_output('cls_prob'),
                                                     net.get_output('bbox_pred'),
                                                     net.get_output('rois'),
                                                     tf.transpose(net.get_output(layer),perm=[3,2,1,0])],
                                                     feed_dict=feed_dict,
                                                     options=run_options,
...
def test_net(sess, net, imdb, weights_filename):
    ...
    for i in range(num_images):
        ...
        for j in range(1, imdb.num_classes):
            ...
            # if we've collected more than the max number of detections,
            # then pop items off the minheap and update the c lass threshold
    ...
    print 'Appying NMS to all detections'
